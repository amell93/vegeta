...
func NewDiyMetrics() *DiyMetrics {
	return &DiyMetrics{
		...
		AllData: make(map[string]*Metrics),
	}
}
...
func (d *DiyMetrics) Add(r *Result) {
	...
	if _, ok := d.AllData[r.Attack]; !ok {
		d.AllData[r.Attack] = &Metrics{}
	}

	d.Data[r.Attack].Add(r)
	d.AllData[r.Attack].Add(r)
}
...
func (d *DiyMetrics) Aggregate() {
	for _, v := range d.AllData {
		v.Close()
	}
}

func (d *DiyMetrics) Reset() {
	d.Data = make(map[string]*Metrics)
}
...
func NewDiyTextReporter(d *DiyMetrics) Reporter {
	...
	return func(w io.Writer) (err error) {
		...
		if Finish {
			...
			err := compute(fmtStr, tw, d.AllData)
			if err != nil {
				return err
			}
		} else {
			fmt.Fprintln(tw, "incremental:")
			err := compute(fmtStr, tw, d.Data)
			if err != nil {
				return err
			}
		}
...
func compute(fmtStr string, tw *tabwriter.Writer, m map[string]*Metrics) (err error) {
	length := len(m)
	i := 0
	for k, m := range m {
